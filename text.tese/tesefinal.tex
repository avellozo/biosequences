...
\linespread{0.8}
\begin{LARGE}
\mbox{    } \mbox{    } \mbox{    }\mbox{    } \mbox{    } \mbox{    }Alinhamento de \seqs \\ \mbox{    } \mbox{    } \mbox{    }\mbox{    } \mbox{    } \mbox{    }com \\  \mbox{    } \mbox{    } \mbox{    }\mbox{    } \mbox{    } \mbox{    }rearranjos
...
\begin{Large}
\mbox{    } \mbox{    } \mbox{    }\mbox{    } \mbox{    } \mbox{    }Augusto Fernandes Vellozo
\end{Large}
...
\end{center}
\vspace{80pt}
\begin{tabular}{p{180pt} l}
 &Este exemplar corresponde à redação final da \\
 &tese devidamente corrigida e defendida \\
 &por Augusto Fernandes Vellozo \\
...
Prof. Dr. Guilherme Pimentel Telles - ICMC/USP

Prof. Dr. Eduardo Sany Laber - PUC-Rio

Prof. Dr. Nalvo Franco de Almeida Júnior - UFMS
...
consider non-over\-lapping inversions, duplications in tandem and duplications
by transpositions.
  ...
  n)$~\cite{MR2173809} time and other, considering some conditions in the scoring
  system, in
...
maior significância à medida que mais segmentos de genomas próximos,
...
Após isto, apresentam um alinhamento ótimo de acordo com algum critério de
...
$O(n^2)$. Esse mesmo problema, sob algumas restrições no sistema de pontuação
do alinhamento, é resolvido pelo algoritmo que propomos
...
outro na \seq\ de DNA. Se a mudança envolve uma purina (\textbf{A}denina ou
\textbf{G}uanina) e uma pirimidina (\textbf{T}imina ou \textbf{C}itosina) então
...
eucariontes. Estima-se que mais de 80\% do genoma de uma planta é composto por
...
do genoma. Outro tipo de repetição muito conhecido é o dito elemento
transponível, ou transposon. Os transposons foram descobertos por Barbara
...
%
%
Identificar repetições de maneira sistemática, e portanto exata, dada uma certa
definição de repetição, é um problema importante em bioinformá\-ti\-ca que
ainda não foi resolvido de maneira satisfatória ou realmente eficaz,
especialmente no caso de genomas de organismos eucariontes. A dificuldade
começa pela grande variedade de tipos de repetições. Algumas repetições muito
compridas (onde cada cópia pode atingir centenas de bases) são
particularmente difíceis de identificar, devido ao comprimento e à pouca
conservação de uma cópia para a outra. Obviamente, o tamanho elevado das
\seqs\ onde as repetições devem ser identificadas aumenta ainda mais o grau de
complexidade do problema.

Algumas doenças humanas são associadas às repetições, tais como: retardamento
...
\label{sec:palavras}
Seja $\alfabeto$ um \emph{alfabeto}, um conjunto de \emph{letras} ou
\emph{símbolos}. Qualquer seqüência finita em $\alfabeto$ é também chamada uma
\emph{palavra em $\alfabeto$} ou simplesmente uma \emph{palavra} se o alfabeto
for claro. Sejam $\alfabeto^*$ o conjunto de todas as palavras em $\alfabeto$,
incluindo a \emph{palavra vazia} denotada por $1$ e $\alfabeto^+$ o conjunto de
todas as palavras em $\alfabeto$, excluindo a \emph{palavra vazia}. Nós
identificamos palavras de comprimento $1$ às letras que elas contém. A
concatenação $\cdot$ de palavras é uma operação associativa definida sobre
$\alfabeto^*$ e será freqüentemente omitida. Seja $w=w[1]w[2]\ldots w[k]$ uma
palavra. Denotamos por $|w|$ o \emph{comprimento} $k$ de $w$. Iremos também
denotar a palavra $w$ por $w[1\Rng k]$. Para $1\leq i\leq j\leq k$, o fator
$w[i]w[i+1]\cdots w[j]$ de $w$ é também representado por $w[i\Rng j]$.
Denotaremos $w[i\Rng j]=1$ se $i>j$. Seja $x,y,z\in \alfabeto^*$. Denotaremos
por $x\alfabeto^*$ o conjunto $\{xy \tq y \in \alfabeto^*\}$, denotaremos por
$\alfabeto^*x$ o conjunto $\{yx \tq y \in \alfabeto^*\}$ e denotaremos por
$\alfabeto^*x\alfabeto^*$ o conjunto $\{yxz \tq y,z \in \alfabeto^*\}$. Diremos
que $x$ é um \emph{prefixo de $w$} se $w \in x\alfabeto^*$, diremos que $x$ é
um \emph{sufixo de $w$} se $y \in \alfabeto^*x$ e diremos que $x$ é um
\emph{fator de $w$} se $w \in \alfabeto^*x\alfabeto^*$.
...
Resolvendo esta recorrência temos que $t(n,m)=O(n\log m + m)$.
...
\begin{enumerate}
  ...
  0 \leq j < i\}$ contém somente posições mortas de $C$ e
...
Grafos de edição são muitas vezes utilizados no desenvolvimento de algoritmos de
comparações de \seqs, como em \cite{96223} e
\cite{DBLP:journals/algorithmica/Meyers86}. Para desenvolver os nossos
algoritmos também utilizamos os grafos de edição. A seguir definimos um
grafo de edição.
...
j))$ e $\egev{(i,j)}=((i-1, j), (i, j))$ de $G$ são as arestas
\emph{horizontal}, \emph{diagonal} e \emph{vertical}, respectivamente, \emph{de
$G$ que chegam em $(i,j)$}.
...
caminho $P$ de $u$ a $v$ é ótimo se ele tiver peso máximo dentre todos os
caminhos de $u$ a $v$. Dizemos que $\egweigp{u}{v}$ é o peso de um caminho
ótimo de $u$ a $v$. Se não houver um caminho de $u$ a $v$ então
$\egweigp{u}{v}=-\infty$.

Repare que vamos considerar como um caminho ótimo num grafo de edição o caminho
com peso máximo, apesar de comumente o caminho de peso mínimo ser considerado
como o caminho ótimo num grafo de edição. Isto comumente é feito pois se deseja
conhecer qual o menor número de operações de edição que diferem uma \seq\ da
outra. Como veremos no próximo capítulo, existe uma relação biunívoca ente um
caminho num \gred\ e um alinhamento. Muitas vezes um biólogo quer obter o grau
de similaridade de duas \seqs\ quando ele faz um alinhamento entre estas \seqs.
Portanto a pontuação de um alinhamento ótimo de duas \seqs\ deve ser
diretamente proporcional à similaridade destas \seqs, ou seja, quanto maior for
a pontuação de um alinhamento ótimo destas \seqs\ mais similares elas devem
ser. Desta forma, vamos tentar obter como um alinhamento ótimo de duas \seqs\
aquele que tem pontuação máxima, ou seja, vamos tentar encontrar um caminho de
peso máximo num \gred.
...
\begin{definicao}[Alinhamento de $s$ e $t$] Sejam $s\in \alfabeto^*$ e
$t\in \alfabeto^*$ duas \seqs\ de comprimentos $n$ e $m$, respectivamente. Um
...
abertura de lacuna, ou \emph{gap open penalty}.
...
Nesta seção iremos mostrar algumas relações de alinhamentos e \opeds. Em
particular, vamos mostrar que para todo alinhamento de $s$ e $t$ podemos
associar uma \seq\ de \opeds\ que transforma $s$ em $t$.
...
Nesta seção iremos mostrar algumas relações de alinhamentos e \greds. Em
particular, vamos mostrar que podemos associar cada alinhamento $A$ de $s$ e
$t$ a um caminho $P$ de $(0,0)$ a $(n,m)$ no grafo de edição de $s$ e $t$ e
vice-versa, ou seja, um caminho num grafo de edição corresponde a um
alinhamento e vice-versa.
...
penalty}) descrito anteriormente. No entanto, freqüentemente neste texto, vamos assumir a
...
máxima\footnote{Definimos os problemas de otimização associados ao problema da
obtenção dos alinhamentos ótimos, como problemas de maximização, pois estamos
tentando obter o grau de similaridade de duas \seqs\ e não a sua distância de
edição, apesar de utilizarmos \greds\ e \opeds\ para obtermos estas
similaridades.} possível. Algumas vezes, pode ser desejável apenas a obtenção
da soma das pontuações das \opeds\ da transformação ótima com \invs\ de $s$ e
$t$, ou seja, a pontuação da transformação ótima com \invs\ de $s$ e $t$.
...
colunas que estão no intervalo $[k_1,k_2]$ são um \emph{trecho de inversão} de
$\aligninvsym$ se $\aligninvsym[2,k_1]=*$, $\aligninvsym[2,k_2]=-$ e não existe
uma coluna $k_2+1$ tal que $\aligninvsym[2,k_2+1]=-$.

O restante desta seção possui aspectos muito técnicos que mostram a relação de
um alinhamento com  \invnsobs\ de $s$ e $t$ e uma \seq\ de \opeds\ que
transforma $s$ em $t$. O restante desta seção mostra também que o peso de um
caminho ótimo em um \gredes\ $G$ de $s$ em $t$ é igual a pontuação de um
alinhamento ótimo com \invnsobs\ de $s$ e $t$, onde o peso da aresta estendida
$\egex{(i',j')}{(i,j)}$ de $G$ é a pontuação de um alinhamento ótimo sem
considerar inversões de $\invert{s[i'\Rng i]} \times t[j'\Rng j]$.
...
$O(n^3)$ e utilizando espaço $O(n^2)$. O algoritmo $O(n^3 \log
n)$ permite maior flexibilidade no sistema de pontuação, mas os sistemas de
pontuação usuais, com pesos inteiros e constantes, se usados no algoritmo
$O(n^3)$ resultam numa complexidade cúbica no tempo de execução.
...
de $G$ tal que $i' \leq i$. Definimos a árvore binária $\arvb{G}{i'}{i}{j}$
da seguinte forma:
\begin{itemize}
...
para obter todos os valores dos $m$ elementos de uma coluna de
$\matw{G}{i'}{i}[j',j]$ é necessário apenas tempo $O(m)$.
...
Para obter um alinhamento com \invnsobs, iremos utilizar o
Algoritmo~\ref{alg:constroiW} sob o \gred\ $\overline{G}$ de $\invert{s}$ e $t$.
...
incremental de $\matw{\overline{G}}{n-i}{n-(i'+1)}$ proposta por Jeanette Schmidt em
...
\geq \egweigp{(i',j')}{(i,j-1)}-g$ e\footnote{Como $\egweigp{(i',j')}{(i,j-1)}
\geq \egweigp{(i',j')}{(i-1,j-1)}-g$ e $\egweigp{(i',j')}{(i,j)}
\leq \egweigp{(i',j')}{(i-1,j-1)}+a$ então $\egweigp{(i',j')}{(i,j-1)}
\geq \egweigp{(i',j')}{(i,j)}-g-a$} $\egweigp{(i',j')}{(i,j-1)}
\geq \egweigp{(i',j')}{(i,j)}-g-a$, portanto
...
$\psihfunc{G}{i'}{i}{j}$ é limitado por uma constante.
...
esparsidade o algoritmo esparso \cite{MR2132586} executa em tempo muito menor
que os outros.
...
Benson para considerar \dups\ em \emph{tandem}, além da restrição de só
considerar \reps\ em \emph{tandem}, tem mais duas restrições, que são as
seguintes.
\begin{enumerate}
  ...
  considera que a \seq\ original da \rep\ pode estar na própria \seq\ onde está
  a \rep.

  \item A \seq\ base deve ser a mesma para \reps\ consecutivas, ou seja,
  como as \dups\ consideradas são em \emph{tandem} e a \seq\ original
  considerada está sempre na outra \seq, as \seqs\ base das \dups\
...
algoritmos exatos que obtêm alinhamentos ótimos com \dups\ e
...
O espaço de busca das \seqs\ base de um \intrep\ utilizado neste modelo, é
maior que o espaço de busca utilizado no modelo proposto por
Benson~\cite{267526}. Contudo a complexidade do tempo de execução $O(n^3)$ do
algoritmo~\ref{alg:dupt} que propomos é muito menor do que as complexidades de
tempo dos algoritmos exatos propostos por Benson, que são $O(n^5)$ e $O(n^4)$.
Além disto, o algoritmo~\ref{alg:dupt} utiliza $O(n^2)$ de memória e o
algoritmo que executa em tempo $O(n^4)$ proposto por Benson, utiliza $O(n^3)$
de memória. Já o algoritmo que executa em tempo $O(n^5)$ proposto por Benson,
assim como o algoritmo~\ref{alg:dupt}, utiliza $O(n^2)$ de memória.

Em relação aos alinhamentos obtidos pelos algoritmos propostos por Benson,
acreditamos que, como verificamos mais possibilidades de eventos de \dups\ então, em
geral, o algoritmo que propomos obterá alinhamentos cujos eventos estão mais
próximos aos ocorridos na natureza.
...
que os já existentes. Desenvolvemos um algoritmo que obtém um alinhamento ótimo
com \invnsobs\ em tempo $O(n^3 \log n)$ e outro que, sob algumas condições no
sistema de pontuação, resolve o mesmo problema
em tempo $O(n^3)$, onde $n$ é o tamanho da maior \seq\ entre as duas sendo
alinhadas. Vale a pena ressaltar que o algoritmo que obtém um alinhamento ótimo
com \invnsobs\ em tempo $O(n^3 \log n)$ tem este tempo de execução mantido para
sistemas de pontuação mais genéricos que os utilizados pelo algoritmo que
...
modelo proposto por Benson~\cite{267526}. Neste trabalho, Benson propôs dois
algoritmos exatos
...
$O(n^4)$ e espaço $O(n^3)$. Propomos um
algoritmo exato para a obtenção de um alinhamento ótimo com \dups\ e outro
para a obtenção de um alinhamento ótimo com \dups\ em \emph{tandem}, sendo que
ambos executam em tempo $O(n^3)$ e espaço $O(n^2)$. Portanto, os
algoritmos propostos por Benson têm complexidades de
tempo e memória muito piores que as dos algoritmos que propomos.

No modelo que propomos para o alinhamento com \dups\ em \emph{tandem}, não
impomos as restrições propostas por Benson e portanto nosso espaço de busca das
\dups\ em \emph{tandem} é maior que o do Benson. É também possível obter um
alinhamento ótimo com as mesmas restrições propostas por Benson com um
algoritmo com tempo de execução $O(n^3)$ e espaço $O(n^2)$ utilizando as
técnicas usadas no algoritmo $O(n^3)$ para alinhamentos ótimos com \invnsobs.
Portanto mesmo considerando o mesmo modelo mais restritivo proposto por Benson,
ainda assim é possível conseguir um algoritmo com complexidade de tempo melhor
que as complexidades dos algoritmos exatos propostos por Benson.

Infelizmente, os algoritmos de alinhamento com \dups\ e com \dups\ em
\emph{tandem} que propomos não puderam ser implementados, de forma que faltam
...
Talvez algumas hipóteses simplificadoras possam ser adotadas, como a não
...
da ocorrência dos ciclos num grafo de \dup\ possa ser minimizado utilizando
dados reais.
...
Além destes algoritmos, também já foram implementados, para fins de análise
