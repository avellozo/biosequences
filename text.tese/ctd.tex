...
DNA. Quando duas seqüências tem alto grau de semelhança é esperado que elas se
...
considerados para um alinhamento são as \emph{substituições}, \emph{remoções} e
...
$s$ e $t$ tal que, a soma das pontuações das \opeds\ desta transformação é
máxima.

Além desses eventos que resultam em alterações em um único símbolo na \seq\
biológica, iremos considerar também os rearranjos para a obtenção de
alinhamentos. Os rearranjos agem sobre um fragmento da \seq. Consideraremos, além
dos eventos de inserção, remoção e substituição de um símbolo, os eventos de
\inv\ e de \dup\ de fragmentos para a obtenção de um alinhamento ótimo de duas
\seqs. Estes não são os únicos tipos de rearranjos, mas são considerados muito
importantes e são largamente estudados, como por exemplo em~\cite{pmid14764619},
% em~\cite{pmid14764619,pmid15286789,pmid15918152,pmid12076675, pmid18039875},
inclusive na associação de doenças humanas à rearranjos no DNA, como por exemplo
em~\cite{pmid1710175}. Os novos resultados comparativos feitos por Sherer et
al.~\cite{pmid16254605} nas \seqs\ de DNA do homem e do chimpanzé, mostram
 a importância do estudo da \inv\ ao nível da \seq\ de DNA e reportaram 83 \seqs\
 reversas que estão contidas dentro de genes.

Consideraremos que $s=s_1s_2\ldots s_n=s[1\Rng n]$ e $t=t_1t_2\ldots t_m=t[1\Rng
m]$ são as duas \seqs\ a serem comparadas e alinhadas.
...
ótima de $s[i'+1 \Rng i]$ em $t[j'+1 \Rng j]$ considerando as operações de
substituição, inserção e remoção.
...
simplificação eles obtiveram um \emph{alinhamento ótimo com \invnsobs} em tempo
$O(n^6)$. A partir daí, outros melhoraram este tempo chegando até $O(n^4)$
...
Diremos que $\invert{s[i'\Rng i]}$ é o complemento reverso de $s[i'\Rng i]$ .
Assumiremos que $ins(t[j])$, $rem(s[i])$, $sub(s[i],t[j])$ e $\opedweiginv$ são,
respectivamente, as pontuações das \opeds\ de inserção de um símbolo $t[j]$,
remoção de um símbolo $s[i]$, substituição de um símbolo $s[i]$ por $t[j]$ e
inversão de um fragmento de $s$.

Os dois algoritmos computam a matriz $B$, onde $B[i,j]$ é a pontuação do
alinhamento ótimo com \invnsobs\ de $s[1\Rng i]$ e $t[1\Rng j]$. O
Algoritmo~\ref{alg:invgeral} contém o pseudo-código seguido pelos dois
algoritmos.

Entre as linhas \ref{invgeral:classicinic} a \ref{invgeral:classicfim} do
Algoritmo~\ref{alg:invgeral} são considerando somentes as \opeds\ de inserção,
...
Os dois algoritmos que propomos diferem somente na forma da construção da
matriz $\matw{G}{i'}{i}$ e do vetor $A_{i'}^i$, onde $\matw{G}{i'}{i}[j',j]$ é a pontuação do
alinhamento ótimo de $\invert{s[i'+1\Rng i]}$ e $t[j'+1\Rng j]$ considerando somente
inserção, remoção e substituição, pois não consideramos inversões sobrepostas no
alinhamento de $s$ e $t$, e $A_{i'}^i[j]$ é o valor máximo de
$(B[i',j]+\matw{G}{i'}{i}[j',j]+\opedweiginv)$, para todo $j'$, $0\leq j' \leq j$.
...
ótimo de $\invert{s[i'+1\Rng i]}$ e $t[j'+1\Rng j]$}
    ...
    \DOFORD {i'} {i} {0}\label{invgeral:loopiLinha}
        ...
        $A_{i'}^i[j]=\max_{j'\leq j}(B[i',j]+\matw{G}{i'}{i}[j',j]+\opedweiginv)$}
...
os valores de $\egweigp{s}{(i',j')}{t}{(i-1,j)}$ o algoritmo precisa de tempo
$O(m \log m)$ para construir outras $m$ árvores que contêm os valores de
$\egweigp{s}{(i',j')}{t}{(i,j)}$.

Pode-se provar que a matriz $\matw{G}{i'}{i}$ é uma matriz de Monge inversa
triangular superior e portanto totalmente monotônica, o que possibilita que
utilizemos o algoritmo proposto por~\cite{MR895444} para obtermos $A_{i'}^i[j]$
com $O(m)$ comparações.

Utilizando a estrutura de árvores proposta
em~\cite{MR1621993} para armazenar os valores de $\egweigp{s}{(i',j')}{t}{(i,j)}$,
executamos a linha~\ref{invgeral:obtemW} do Algoritmo~\ref{alg:invgeral} em tempo
$O(m \log m)$ e, utilizando o algoritmo proposto por~\cite{MR895444}, obtemos
$A_{i'}^i[j]$ na linha~\ref{invgeral:obtemA} em tempo $O(m \log m)$. Como essas
linhas são executadas para todo $i$ e $i'$, $0 \leq i' \leq i \leq n$, o
Algoritmo~\ref{alg:invgeral} é executado em tempo  $O(n^2m \log m)$. Além
disto, pode-se verificar que o Algoritmo~\ref{alg:invgeral} precisa de
$O(\max(nm, m^2))$ de memória.
...
Uma duplicação é um evento biológico que ocorre frequentemente e que insere uma
cópia de um fragmento da \seq\ na própria \seq. Existe um tipo de
duplicação muito estudado que são as duplicações encadeadas (ou \dups\
\emph{tandem}). Neste tipo de \dup\ a cópia é inserida imediatamente após (ou
antes) do fragmento copiado. Diremos que repetição é a cópia inserida e
\seq\ base é a \seq\ de onde a repetição foi copiada.

Em~\cite{267526} é proposto um modelo para o alinhamento de \seqs\ que
considera \dups\ em \emph{tandem} de mesmo tamanho. São propostos
dois algoritmos exatos para obter um tal alinhamento ótimo que considera estas
\dups. O primeiro algoritmo proposto executa em tempo $O(n^5)$ e espaço
$O(n^2)$. O segundo algoritmo proposto executa em tempo $O(n^4)$ e espaço
$O(n^3)$. Propomos algoritmos que executam em tempo $O(n^3)$ e memória $O(n^2)$.

Propomos um modelo para obter um alinhamento ótimo com \dups\ que é parecido
com o modelo proposto por Benson, porém com algumas restrições diferentes:
\begin{enumerate}
 \item Benson propõe somente \dups\ em \emph{tandem}. Não mantemos a
restrição de ser em \emph{tandem}. Um modelo com esta restrição também pode ser
considerado sem alterar as complexidades do algoritmo que propomos.
  \item No modelo proposto por Benson, se um trecho de $s$ é uma
  \rep\ então a \seq\ base a ser comparada (ou alinhada) com esta \rep\
  deve ser um trecho de $t$. Portanto ele não
  considera que a \seq\ base da \rep\ pode estar na própria
  \seq. Nós consideramos que a \seq\ base pode estar na outra
  \seq. Com isto, se estamos interessados em obter uma transformação ótima com
  \dups\ de $s$ em $t$, o modelo proposto por Benson
considera que as \opeds\ pontuais (inserção, remoção e subsituição) em um
trecho duplicado devem ocorrer depois da \dup\ na transformação. Nós
consideramos que as \opeds\ pontuais podem ocorrer antes ou depois da \dup.
  ...
  trecho das \seqs\ $s$ ou $t$.
\end{enumerate}

Se estamos obtendo uma transformação ótima de $s$ em $t$ com \dups\ então
consideramos que a \seq\ base de uma \rep\ $s[i+1\Rng i']$ está em
$s[1\Rng i]$ ou $t[1\Rng j]$, onde $s[1\Rng i]$ se transforma em $t[1\Rng j]$ na
transformação ótima sendo obtida. Além disto, em uma transformação com
\dups\ consideramos a existência da \oped\ de \emph{excisão} que é a \oped\ dual
à \dup, ou seja, uma excisão remove um trecho da \seq\ (\rep), tal que a \seq\ de símbolos do trecho removido ainda
aparece como um fragmento (\seq\ base) da \seq\ gerada após a excisão. Numa
transformação de $s$ em $t$, podemos associar uma \oped\ de excisão ao
evento \bio\ de uma \dup\ na \seq\ $s$ que não ocorreu em $t$ ou ao evento \bio\ de
remoção de uma \rep\ em $t$ que não ocorreu em $s$.
...
Benson, em tempo $O(n^3)$ e com memória $O(n^2)$. Porém, acreditamos que o
modelo que propomos está mais próximo à realidade, pois considera mais
casos de \dups.
...
se $0 \leq i' < i \leq n$ e $0$ caso contrário, onde $\seqbaseSym \in
...
$t[1\Rng m]$, onde $s[1\Rng i]s[i'+1\Rng n]$ é a concatenação das \seqs\
$s[1\Rng i]$ e $s[i'+1\Rng n]$. De forma análoga, definimos a matriz \dupwsym{t}.
Utilizando-se apenas de programação dinâmica conseguimos construir as matrizes
\dupwsym{s} e \dupwsym{t} em tempo $O(n^3)$ e memória
$O(n^2)$.

Seja $M$ uma matriz $n\times m$ tal que
\begin{equation}
\begin{array}{l}
M[0,0]=0\\
...
Diremos que $M[i,j]$ é a pontuação de um alinhamento ótimo com \dups\ de $s[1\Rng
i]$ e $t[1\Rng j]$.
Dadas as matrizes \dupwsym{s} e \dupwsym{t} existe um algoritmo que com
programação dinâmica constrói a matriz $M$ em tempo $O(n^3)$ e memória
$O(n^2)$.
...
inserção, remoção e substituição, comumente utilizados em
algoritmos usuais para obtenção de alinhamentos ótimos. Esperamos com isto, que o
...
modelo que propomos para alinhamentos com \dups\ em \emph{tandem} é o modelo proposto
por Benson~\cite{267526}. Porém, os algoritmos propostos por Benson têm
complexidades de tempo e memória muito piores que as dos algoritmos que propomos,
apesar do nosso espaço de busca das \dups\ em \emph{tandem} ser maior que o do
Benson.
