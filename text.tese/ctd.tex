...
Diremos que $\egweigp{(i',j')}{(i,j)}$ é a pontuação de uma transformação ótima
de $s[i'+1 \Rng i]$ em $t[j'+1 \Rng j]$ considerando as operações de
...
de $\egweigp{(i',j')}{(i,j)}$ em tempo $\log m$. Dada a estrutura de árvores com
os valores de $\egweigp{(i',j')}{(i-1,j)}$ o algoritmo precisa de tempo $O(m \log
m)$ para construir outras $m$ árvores que contêm os valores de
$\egweigp{(i',j')}{(i,j)}$.
...
em~\cite{MR1621993} para armazenar os valores de $\egweigp{(i',j')}{(i,j)}$,
...
Se estamos interessados em obter uma transformação ótima de $s$ em $t$ com \dups\
então no nosso modelo consideramos que a \seq\ base de uma \rep\ $s[i+1\Rng i']$
está em $s[1\Rng i]$ ou $t[1\Rng j]$, onde $s[1\Rng i]$ se transforma em
$t[1\Rng j]$ na transformação ótima sendo obtida. Além disto, consideramos a
existência da \oped\ de \emph{excisão} que é a \oped\ dual à \dup, ou seja, uma
excisão remove um trecho da \seq, tal que a \seq\ de símbolos do trecho removido
ainda aparece na \seq\ gerada após a excisão. Numa transformação de $s$ em $t$
com excisão, podemos associar uma excisão ao evento \bio\ de uma \dup\ na \seq\
$t$ ou ao evento \bio\ de remoção de uma \rep\ em $s$.

Vale a pena ressaltar que, utilizando as técnicas utilizadas em~\cite{MR1869256},
podemos obter um alinhamento ótimo com \dups\ em \emph{tandem} segundo o
mesmo modelo proposto por Benson, em tempo $O(n^3)$ e com memória $O(n^2)$.
Porém acreditamos que os modelos que propomos são modelos mais reais, pois consideram mais casos de
\dups.

\subsection{Algoritmo para alinhamento com \dups}
