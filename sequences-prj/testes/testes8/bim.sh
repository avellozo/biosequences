...
for i in $(seq 20); do time java -Xms64m -cp sequences.jar sequences/ui/Bim -i 2 -t 45 -b --time -f testes/XF02270239-PA47274746-window100-displacement100-r2-q1-G0-E10.compmat -m dummy -w testes/weights100 >> testes/xf100wdummy.txt; done;
for i in $(seq 20); do time java -Xms64m -cp sequences.jar sequences/ui/Bim -i 2 -t 45 -b --time -x testes/xae1.seq -y testes/xcc2.seq -m dummy >> testes/xae140dummy.txt; done;
