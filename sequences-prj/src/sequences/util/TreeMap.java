 ...
 * mappings; merely changing the weight associated with an existing key is not
                     ...
                     implements SortedMap, Cloneable, java.io.Serializable
{
     ...
     * weight)</tt> call will throw a <tt>ClassCastException</tt>.
     *
     * @see Comparable
     */
    public TreeMap() {
     ...
     *        <tt>null</tt> weight indicates that the keys' <i>natural
...
//
    ...
    /**
     * Returns the number of key-weight mappings in this map.
     *
     * @return the number of key-weight mappings in this map.
     */
    public int size() {
        return size;
    }
     ...
     * specified weight.  More formally, returns <tt>true</tt> if and only if
     * this map contains at least one mapping to a weight <tt>v</tt> such
     * that <tt>(weight==null ? v==null : weight.equals(v))</tt>.  This
     * operation will probably require time linear in the Map size for most
     * implementations of Map.
     *
     * @param weight weight whose presence in this Map is to be tested.
     * @return  <tt>true</tt> if a mapping to <tt>weight</tt> exists;
    ...
    /**
     * Returns the weight to which this map maps the specified key.  Returns
     * <tt>null</tt> if the map contains no mapping for this key.  A return
     * weight of <tt>null</tt> does not <i>necessarily</i> indicate that the
     * map contains no mapping for the key; it's also possible that the map
     * explicitly maps the key to <tt>null</tt>.  The <tt>containsKey</tt>
     * operation may be used to distinguish these two cases.
     *
     * @param key key whose associated weight is to be returned.
     * @return the weight to which this map maps the specified key, or
     ...
     * @throws    ClassCastException class of a key or weight in the specified
...
//
    ...
    /**
     * Associates the specified weight with the specified key in this map.
     * If the map previously contained a mapping for this key, the old
     * weight is replaced.
     *
     * @param key key with which the specified weight is to be associated.
     * @param weight weight to be associated with the specified key.
     * 
     * @return Entry object created.
     * 
     * @throws    ClassCastException key cannot be compared with the keys
     *            currently in the map.
     * @throws NullPointerException key is <tt>null</tt> and this map uses
        ...
        while (true) {
            ...
            if (cmp == 0) {
                ...
                if (t.left != null) {
                    ...
                    incrementSize();
                    e = new Entry(key, value, t);
                    t.left = e;
                    e.next = t;
                    e.previous = t.previous;
                    e.previous.next = e;
                    e.next.previous = e;
     ...
     * @return previous weight associated with specified key, or <tt>null</tt>
...
//
     ...
     * key-weight mappings in <tt>m</tt> whose keys are between <tt>low</tt>
     * and <tt>high</tt>, inclusive:
     *             <pre>    SortedMap sub = m.submap(low, high+"\0");</pre>
     * A similar technique can be used to generate an <i>open range</i> (which
     * contains neither endpoint).  The following idiom obtains a view
     * containing all of the key-weight mappings in <tt>m</tt> whose keys are
     ...
     * the key-weight mappings in <tt>m</tt> whose keys are less than or equal
     * to <tt>high</tt>:
     * <pre>
     ...
     * the element type allows for calculation of the successor a given weight,
     ...
     * all of the key-weight mappings in <tt>m</tt> whose keys are strictly
    ...
    /**
     * Node in the Tree.  Doubles as a means to pass key-weight pairs back to
     * user (see Map.Entry).
     */
    ...
    public static class Entry implements Map.Entry {
        ...
        /**
         * Make a new cell with given key, weight, and parent, and with 
        ...
        /**
         * Returns the weight associated with the key.
         *
         * @return the weight associated with the key.
         */
        ...
        /**
         * Replaces the weight currently associated with the key with the given
         * weight.
         *
         * @return the weight associated with the key before this method was
         *           called.
         */
...
//        return p;
    ...
    protected Entry successor(Entry t) {
        if (t == null)
            return null;
    	return (t.next == HEADER ? null : t.next);
...
//        }
     ...
     */
    protected Entry predecessor(Entry t) {
        if (t == null)
            return null;
    	return (t.previous == HEADER ? null : t.previous);
//        else if (t.left != null) {
//            Entry p = t.left;
//            while (p.right != null)
//                p = p.right;
...
//        }
     ...
     * serialize it).
     *
     * @serialData The <i>size</i> of the TreeMap (the number of key-weight
     *             mappings) is emitted (int), followed by the key (Object)
     *             and weight (Object) for each key-weight mapping represented
     *             by the TreeMap. The key-weight mappings are emitted in
...
//    }
     ...
     *
     * @param size the number of keys (or key-weight pairs) to be read from
     ...
     * @param defaultVal if non-null, this default weight is used for
     *        each weight in the map.  If null, each weight is read from
...
//                                   it, str, defaultVal);
//        
//        // extract key and/or weight from iterator or stream
//        Object key;
//        Object value;
